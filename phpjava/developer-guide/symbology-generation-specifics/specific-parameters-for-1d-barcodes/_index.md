...
## **Bar Filling Modes**
For 1D barcodes, ***Aspose.BarCode for Android via PHP via Java*** provides a specific mode to generate barcodes with empty bars instead of filled ones. Such a modification can be done using the *setFilledBars* method of class [*BarcodeParameters*](). This property is set to *True* by default and is valid only for 1D barcodes. 
