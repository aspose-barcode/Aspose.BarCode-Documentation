...
Aspose.BarCode provides an X-dimension property in BarcodeGenerator classes that allow developers to set the X-dimension of the bars in the barcode. Normally, X-dimension is configured in Mils but to ease the job of developers, Aspose.BarCode uses Millimeter as a measuring unit by default. But of course, developers can modify the measuring unit according to their choice using defined units under [Unit ](https://reference.aspose.com/barcode/cpp/class/aspose.bar_code.generation.unit)class. Just like X-dimension in linear barcodes, Y-dimension can be achieved by using the BarCodeHeight property of BarcodeGenerator class for 2D barcodes.
