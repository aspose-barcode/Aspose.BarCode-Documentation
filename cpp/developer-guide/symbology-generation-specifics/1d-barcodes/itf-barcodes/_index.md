...
In ***Aspose.BarCode for C++***, developers can set five different styles for *ITF* barcode frames, including the absence of such. It can be done by using the *ItfBorderType* enumeration that provides the following options: *None*, *Frame*, *Bar*, *FrameOut*, and *BarOut*. Note that *FrameOut* and *BarOut* allow placing the frame outside a barcode image without affecting its original height. 
...
## **Quiet Zone Settings**
In ***Aspose.BarCode for C++***, developers can customize the size of the quiet zone in *ITF* barcodes by setting the *QuietZoneCoef* property of class *ITFParameters*. This property is defined as a relative coefficient to the *XDimension* parameter.  
