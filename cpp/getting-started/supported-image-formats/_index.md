...
weight: 20
feedback: BARCODECOM
url: /cpp/image-formats/
aliases:
- /cpp/supported-file-formats/
---
  
***Aspose.BarCode for C++*** enables outputting barcode images using five most common raster image file formats and two vector image formats. The latter ones are available for rendering only by the SQL Server Reporting Services engine. Image formats supported for input and output are listed below.
