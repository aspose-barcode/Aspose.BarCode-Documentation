...
-	*Utf8BOM* and *Utf16BEBOM*. These modes are applied to encode input data using UTF8 and UTF16BE encodings, respectively; a byte order mark (BOM) character is added to indicate the used encoding. Note that it is preferable to use the *ECIEncoding* mode as it allows setting the encodings that are explicitly specified in the *QR Code* standard.  
...
### ***Auto*** Mode
The *Auto* data encoding mode utilizes the [*CodeTextEncoding*](https://apireference.aspose.com/barcode/net/aspose.barcode.generation/qrparameters/properties/codetextencoding) property to encode input data. In case if the encoding mode cannot be defined using the value of *CodeTextEncoding*, the encoding mode similar to *Bytes* is applied. The code snippet provided below demonstrates how to use this mode.
...
The *QR Code* family supports several levels of Reed-Solomon error correction. This mechanism implies adding redundant information to a barcode but allows detecting and correcting errors automatically in the case of barcode label damage. In general, to restore 1% of errors, 2% redundancy is required.  
